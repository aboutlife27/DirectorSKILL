...
Load this file when the project has a genre or format, after dramatic evidence, the film constitution, arc position, and scene archetype are known. Genre supplies defaults only where those higher constraints are silent.

## How to use a playbook

A playbook is a set of pre-decided defaults for one genre. It fires at **Step 7 (Director's book)**, is written into Mode C, and carries through Step 8 (blocking), Step 9 (shot list), Step 12 (video prompts), Step 13 (sound), and Step 14 (edit). It does not replace craft reasoning — it pre-decides the twelve items every block below carries, so you argue them once per project instead of once per scene.

Precedence, highest wins:

1. Dramatic evidence from the director read.
2. Film visual constitution and current visual-arc position.
3. The selected scene archetype.
4. Explicit user functional preference that fits the first three levels.
5. This file's genre defaults.
6. Skill-level defaults in `SKILL.md`.

A named-director reference never owns a field. Translate it into a functional method, test it against levels 1–3, and reject or restate it when it conflicts. Do not load a complete style package on top of this playbook.
...
- **The film constitution owns the craft boundaries.** Camera, movement, lighting, palette, blocking, sound and pacing stay inside its allowed families; the arc controls how their emphasis changes.
- **Dramatic evidence owns the dramaturgy.** The genre may suggest an engine and audience contract, but the actual script can override both with a logged reason.
- **Delivery format owns aspect and clip duration.** A functional composition method that assumes another aspect must be restated for the delivery aspect and logged in the director's book; it is never pasted through unchanged.
...
Worked, format × functional reference. A method that isolates a standing figure at 8–15% of frame height with the camera 12–22 m back cannot hold that number at 9:16. Frame-height fraction is a function of vertical field of view, so at native vertical the same figure at the same distance on the same focal reads at roughly half the fraction, and [cinematic-language.md](cinematic-language.md) is explicit that an extreme wide in vertical wastes sky and floor and leaves the subject a few pixels tall. Preserve the function by restating it: at 9:16 produce smallness through depth down a vertical subject — stair core, ramp, parking-deck run, alley — and by glass-reflection layering that puts the city over the face, never by lateral distance. Keep the 8–15% target only in genuinely vertical subjects; everywhere else move one size step tighter.

Field key, in the order every block uses it: **dramatic engine** (what mechanically generates scenes) · **audience contract** (what the viewer is owed) · **camera** (default framing register, plus **lens bias** in mm) · **movement bias** · **lighting** (quality, motivation, and **key ratio**（布光比）) · **palette** · **pacing** (**ASL**, the average-shot-length range, and the **pacing signature**, kept on one line because they are read together; the arithmetic lives in [editing-and-assembly.md](editing-and-assembly.md)) · **sound** · **blocking bias** · **classic amateur mistake** · **AI risk + mitigation** · **seed prompt** to start Step 12 from.

Twelve bullets, nine of which are settings. Two are dramaturgical — engine and contract. Seven are craft dials: **camera, movement, lighting, palette, pacing, sound, blocking**. The remaining three (mistake, risk, seed) are notes. Count moved dials against that seven whenever you deviate. Social vertical carries two extra lines — `contract under a reference`, because format rules often expose conflicts in a borrowed method, and `platform grammar`, because the Chinese-platform vocabulary has no equivalent elsewhere in this file.
...
- Contract under a reference: structural obligations always survive — a hook inside the first second, a visible change by 3 s, and a reason to keep watching at the loop point. A borrowed method may be restated for sound-off legibility, captions, saturation or front-biased light, but it never arbitrates against the audience contract or film constitution. Captions are a Step 14 delivery decision, not a generated-image craft dial. Log every restatement in the director's book.
